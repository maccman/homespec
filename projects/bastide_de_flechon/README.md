...
**All 54 final Cycles stills and the local portable model are complete.** The image-only delivery verifier passed **3,408 assertions** and the independent local audit passed **966**. All 54 images have a recorded visual review. Video is omitted at the user's request. [Final delivery and evidence](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/final-delivery.md).

The separate [original-photo audit](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/original-photo-audit.json) passes 128 checks: all 61 cached photographs match the supplied ZIP bytes, and all 12 comparison-pair mappings are retained.
...
Website downloads use **54 full-resolution 8-bit RGB JPEG exports**, quality **96**, **4:4:4** chroma sampling, with **no resize or crop**. They preserve each rendered composition and pixel dimensions; the original 16-bit PNGs remain local. The [export manifest](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/site-jpeg-fallback/manifest.json) binds every JPEG to its unchanged PNG source; SHA-256 `f74e33bc85ce8b16bba6ba82dffb2499dff8a87a676ca608bcd3ab7106e964fe`.

## Cameras and source

The [v5 interior lock](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/photo_camera_lock.json) and [exterior register](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/exterior_cameras.json) are unchanged. Five close-ups use the [recorded detail cameras](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/delivery_detail_cameras.json). Only gallery/navigation view 11, Garden bedroom one, changes its look direction to include the bed/artwork; its position, 24 mm lens, geometry and photograph locks remain unchanged. [Exact framing change](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/bookmark-framing-change.json). Historical baselines retain their own camera/source provenance.
...
[Exact package source and hashes](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/deliverables/SOURCE.json) · [Saved-scene checks](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/scene-validation-summary.json) · [Exterior light-record reconciliation](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/exterior-light-snapshot-reconciliation.json). [Whole-delivery receipt](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/deliverables/artifact-verification.json). Earlier focused reviews remain historical evidence in the [preserved README](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/README-review-history.md).

## Open the local portable model

The verified model and ZIP are **local only, excluded from website upload at the user's request**. Open the [local ZIP](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/deliverables/La-Bastide-de-Flechon-Walkthrough.zip), extract it and keep the folder together. With Blender 5.2.1 installed, macOS users can open **Walk Bastide.command**. Elsewhere run `blender house_walk.blend --python walk_ui.py` from the extracted folder. In Blender's Flechon sidebar choose a room and click **Walk from here**; mouse looks, WASD moves, Q/E moves vertically, Shift speeds up, Enter finishes and Esc cancels. N shows the sidebar.

The packed model has 26 checked bookmarks, ten Eevee previews, three light probes and 64 embedded image datablocks with no external textures. The source has three additional zero-user image blocks omitted from the portable save; 67 is the pre-save pack-log count. All 18 ZIP members pass byte/CRC/permission checks, including executable launcher mode. [Portable audit](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/portable-final-package-audit.json).

## Checks and remaining limits

The native build records **375 passes and one acknowledged `bed3` glazing guideline failure: 0.086 against 0.1**. The plan/photo-supported opening is retained; thresholds are unchanged. The native report retains **55 architectural clash records**; the separate dressed-scene audit retains **46 raw findings: 43 inside-wall and three guest-bedroom door-route reports**. These are not erased by artifact checks. Rubble still intersects dressed trim in the shutter/sill close-up. Gallery view 18 retains a thin irregular inner-left arch boundary and upper arch facets, also present in its approved preview. Roof/oculus heights, courtyard terrace levels, door states, tight approaches and inferred furnishings retain documented differences; this is a photographic reconstruction, not a measured scan or whole-house collision certification. [Exterior limits](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/exterior-discrepancies.md) · [Interior limits](/Users/cloud/.codex/worktrees/49bf/homespec/projects/bastide_de_flechon/photo-discrepancies.md).

## Reproduce the final images

Use the existing locked `.venv` from the repository root and obtain the shared exclusive Blender worker before running these sequential jobs; these commands do not acquire it. Keep source, dependencies and the frozen scene unchanged. The resolver rejects stale inputs; rebuild and recheck first if they change. Preserve historical baselines. Producers reject incompatible existing manifests; use a fresh delivery destination when identities/settings change.

```sh
set -eu
cd /Users/cloud/.codex/worktrees/49bf/homespec
...
.venv/bin/python "$FLECHON_PROJECT/package_model.py"
...
.venv/bin/python "$FLECHON_PROJECT/verify_delivery.py" --images-only
```

Pillow runs in the isolated `uv --with` overlay; `--no-sync --frozen` preserves the build's `.venv` fingerprint. Package creation also uses Blender and the same exclusive worker. The completed visual review and retained discrepancies accompany this release. The image-only verifier still requires every still/control/material image and web asset, plus the local native model check and ZIP. The canonical Site helper is `/Users/cloud/bastide-comparison-site-render/scripts/prepare-final-assets.py`. Local delivery and website publication are complete. All 592 deployed HTTP checks passed; see the [publication receipt](/Users/cloud/.codex/worktrees/49bf/homespec/out/final-delivery/site-publication.json). The original 16-bit PNGs, model and ZIP stay local.
